  ...
  return (
    <Dialog.Root>
      <Dialog.Trigger className=" rounded-md text-left flex flex-col bg-slate-800 p-5 gap-3 overflow-hidden relative hover:ring-2 hover:ring-slate-600 outline-none focus:ring-2 focus:ring-lime-400">
        ...
        <div className="absolute bottom-0 left-0 right-0 h-1/2 bg-gradient-to-t from-black/60 to-blank/0 pointer-events-none" />
      </Dialog.Trigger>
      <Dialog.Portal>
        <Dialog.Overlay className="inset-0 fixed bg-black/50">
          <Dialog.Content className="fixed inset-0 md:inset-auto md:left-1/2 md:top-1/2 md:-translate-x-1/2 md:-translate-y-1/2 md:max-w-[640px] w-full md:h-[60vh] bg-slate-700 md:rounded-md flex flex-col outline-none overflow-hidden">
            ...
            <button
              type="button"
              className="py-4  bg-red-700 hover:bg-opacity-80 group"
              onClick={() => {
                onDeleteNote(note.id);
              }}
            >
              <div className="antialiased group-hover:scale-110 uppercase duration-200 font-medium transition ease-out ">
                apagar
              </div>
            </button>
          </Dialog.Content>
